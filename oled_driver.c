...
// writes to the gpioa data pin states
void digitalWrite(uint8_t pin, uint8_t state) {
	// make sure it's only one bit
	state &= 1;
	int offset = pin + 16 * (state ? 0 : 1);
	GPIOA->BSRR |= 1 << offset;
}
...
//  }
  ...
  GPIOA->MODER &= ~(0x3 << (2 * dc));
  GPIOA->MODER &= ~(0x3 << (2 * cs));
  ...
  for (uint16_t x = 0; x < 128; x += 2) {
    for (uint16_t y = 0; y < 64; y += 8) { // we write 8 pixels at once
      ...
      for (uint8_t p = 0; p < 8; p++) {
        ...
        if (right8 & (1 << p))
          d |= 0x0F;
        // this could definitely be wrong; may need to re-implement the spixer function
//        TM_SPI_Send(SPI1, d);
        spixfer(d);
      }
    }
